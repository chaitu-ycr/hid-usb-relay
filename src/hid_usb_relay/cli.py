    ...
    def run(self, argv: Sequence[str] | None = None) -> int:
        ...
        try:
            if args.command == 'devices':
                ...
                result = self.service.get_state(args.relay_id, args.relay_number) if args.command in ('state', 'status') else self.service.set_and_get_relay_state(args.relay_id, args.relay_number, args.relay_state)
                self._print_json({'relay_number': args.relay_number, 'relay_id': args.relay_id, 'relay_state': result.relay_state})
            return 0
